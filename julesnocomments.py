...
# PHYSICS CONSTANTS
# -------------------------------------------------------------------------------------------------
# The radius of the pool ball in pixels. This is critical for accurate collision detection.
# When a ball hits a rail, its CENTER stops exactly one radius away from the physical rail.
# If we don't account for this, the visual line will look like it overlaps the wood.
BALL_RADIUS = 18 

# Table Rail Coordinates
# These coordinates define the PHYSICAL boundaries of the pool table cushions.
...
# These variables define the absolute maximum outer limits of the playable area.
...
# This allows us to quickly check "Is this point near a rail?" without doing complex math.
RAIL_BOUNDS = []
RAIL_TOLERANCE = 2.0
...
def extend_line_to_rails(line, ghostball_center):
    ...
    CRITICAL UPDATE: This function now accounts for BALL_RADIUS (18px).
    The aim line stops when the *edge* of the ball hits the rail, which means the 
    center of the ball (the end of our line) stops 18px away from the rail.
    ...
    # -------------------------------------------------------------------------
    # 1. Determine direction: Ghostball -> Furthest Point on line
    # -------------------------------------------------------------------------
    # We calculate squared distance to both endpoints to see which is the "tip" of the aim.
    # This prevents the line from accidentally pointing backwards if the detected segment
    # is inside the ghostball.
    ...
    # Calculate vector components from Ghostball to Target
    dx = target_x - gb_x
    dy = target_y - gb_y
    ...
    if length < 1e-6:
        # Avoid division by zero if line length is effectively 0
        return (gb_x, gb_y, target_x, target_y)

    dx_norm = dx / length
    dy_norm = dy / length

    # -------------------------------------------------------------------------
    # 2. Calculate EFFECTIVE Table Boundaries (Physical - BallRadius)
    # -------------------------------------------------------------------------
    # Since we are tracking the ball's center, we must shrink the collision box
    # by the radius of the ball. The center cannot get closer than 18px to any wall.
    eff_min_x = TABLE_MIN_X + BALL_RADIUS
    eff_max_x = TABLE_MAX_X - BALL_RADIUS
    eff_min_y = TABLE_MIN_Y + BALL_RADIUS
    eff_max_y = TABLE_MAX_Y - BALL_RADIUS

    # Ray Casting: Find intersection with the four planes defining the effective table box
    # Parametric line equation: P = P0 + t * V
    t_values = []

    # Check Left Wall (effective)
    # Only check if we are moving left (dx is negative)
    if dx_norm < -1e-6:
        t = (eff_min_x - gb_x) / dx_norm
        if t > 0: t_values.append((t, 'left'))
            
    # Check Right Wall (effective)
    # Only check if we are moving right (dx is positive)
    if dx_norm > 1e-6:
        t = (eff_max_x - gb_x) / dx_norm
        if t > 0: t_values.append((t, 'right'))

    # Check Top Wall (effective)
    # Only check if we are moving up (dy is negative)
    if dy_norm < -1e-6:
        t = (eff_min_y - gb_y) / dy_norm
        if t > 0: t_values.append((t, 'top'))

    # Check Bottom Wall (effective)
    # Only check if we are moving down (dy is positive)
    if dy_norm > 1e-6:
        t = (eff_max_y - gb_y) / dy_norm
        if t > 0: t_values.append((t, 'bottom'))

    # If no intersections found (unlikely), return line from ghostball to target
    if not t_values:
        return (gb_x, gb_y, target_x, target_y)

    # The closest intersection (smallest positive t) is the one we hit first
    t_min, side = min(t_values, key=lambda x: x[0])

    # Calculate the exact intersection point (Where the center of the ball stops)
    intersect_x = gb_x + t_min * dx_norm
    intersect_y = gb_y + t_min * dy_norm

    # -------------------------------------------------------------------------
    # 3. Check if this collision is a RAIL or a POCKET
    # -------------------------------------------------------------------------
    # We do this by checking the PHYSICAL coordinate corresponding to this hit.
    # Example: If we hit the 'left' effective wall, we check if the physical point
    # on the left rail (18px further out) is a valid rail segment.
    is_rail_hit = False
    
    if side == 'left':
        # Check against physical Left Rail X (TABLE_MIN_X)
        is_rail_hit = is_on_rail((TABLE_MIN_X, intersect_y))
    elif side == 'right':
        # Check against physical Right Rail X (TABLE_MAX_X)
        is_rail_hit = is_on_rail((TABLE_MAX_X, intersect_y))
    elif side == 'top':
        # Check against physical Top Rail Y (TABLE_MIN_Y)
        is_rail_hit = is_on_rail((intersect_x, TABLE_MIN_Y))
    elif side == 'bottom':
        # Check against physical Bottom Rail Y (TABLE_MAX_Y)
        is_rail_hit = is_on_rail((intersect_x, TABLE_MAX_Y))

    if is_rail_hit:
        # It's a valid rail bounce. Stop exactly at the effective boundary.
        # This makes the line look like it touches the ball edge to the rail.
...
def main():
    ...
    with mss.mss() as sct:
        ...
        while True:
            ...
            # SAFEGUARD: Cast coordinates to int to prevent OpenCV errors if float
            # We use the raw TABLE_MIN/MAX here because we want to mask the whole table
            cv2.rectangle(play_area_mask,
                         (int(TABLE_MIN_X), int(TABLE_MIN_Y)),
                         (int(TABLE_MAX_X), int(TABLE_MAX_Y)),
            ...
            if ghostball_center is not None:
                ...
                if roi_hsv is not None and roi_rect is not None:
                    ...
                    if line is not None:
                        ...
                        # This now uses the updated physics logic (ball radius + vector fix)
